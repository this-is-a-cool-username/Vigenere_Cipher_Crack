...
#configure logging
logging.basicConfig(filename='vigenere.output', level=logging.INFO, format="%(message)s",)
...
def crack(cipher):
    ...
    for keyLen in range(2, 10): # goes through the the key guesses (if you enable the debug in this function you can see how many keys its goes through)
        ...
        logging.info(f"Potential key: {keyGuess}\nScore: {score}\nPlaintext: {plain}\n")
        if score < bestScore:
            bestScore = score
            bestPlaintext = plain
            bestKeyGuess = keyGuess
    logging.info("=========================================================\n")
    logging.info("Most likely solution:")
    logging.info(" Key: %s", bestKeyGuess)
    logging.info(" Plaintext: %s", bestPlaintext)
    logging.info("~End \n\n\n\n\n\n\n\n\n\n\n")

# This is what happens when you run the program
cipherText = input("Input cipher text to decrpyt: ")
logging.info("~Begin")
logging.info("Ciphertext: %s\n",cipherText)
crack(cipherText)